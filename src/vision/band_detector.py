...
class BandDetector:
    ...
    def detect_bands_projection(self,roi):
        ...
        # OpenCV needs a 2D array for thresholding and finding contours
        ...
        _, thresh = cv2.threshold(diffs_2d, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        temp_cnts, _ = cv2.findContours(thresh.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if len(temp_cnts) < 3:
            _, thresh = cv2.threshold(diffs_2d, 20, 255, cv2.THRESH_BINARY)
